document.addEventListener('DOMContentLoaded', function () {
    ...
    inputCodigo.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') {
            ...
            fetch(`/buscar-producto-codigo/?codigo=${encodeURIComponent(codigo)}&tipo=farmacia`)
    ...
    function renderProducto(producto) {
        ...
        tbody.innerHTML = `
            <tr class="hover:bg-gray-200 rounded-lg transition-colors">
                ...
                <td class="p-3 text-xl text-center">${producto.presentacion || '-'}</td>
...
function filtrarProductos() {
    ...
    if (!nombre && !categoria && !presentacion) {
        window.location.href = '/Productos/listar'
    }

    fetch(`/filtrar_productos/?nombre=${nombre}&categoria=${categoria}&presentacion=${presentacion}`)
        .then(response => response.json())
        .then(data => {
            const tbody = document.querySelector("#datos tbody");
            tbody.innerHTML = "";
                ...
                data.forEach(producto => {
                    tbody.innerHTML += `
                        <tr class="hover:bg-gray-200 rounded-lg transition-colors">
                            ...
                            <td class="p-3 text-xl text-center">${producto.presentacion}</td>
                            <td class="p-3 text-xl text-center">${producto.stock}</td>
                            <td class="p-3 text-xl text-center">${producto.preciounidad}</td>
                            <td class="p-3 text-xl text-center">
                                <div class="grid grid-cols-2 gap-2 w-full">
                                    <!-- tus botones aquí -->
                                    <a href="#" onclick="window.location.href='actualizarProducto/${producto.productoid}'" class="btn">
