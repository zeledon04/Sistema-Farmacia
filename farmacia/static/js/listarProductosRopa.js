document.addEventListener('DOMContentLoaded', function () {
    ...
    inputCodigo.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') {
            ...
            fetch(`/buscar-producto-codigo/?codigo=${encodeURIComponent(codigo)}&tipo=tienda`)
    ...
    function renderProducto(producto) {
        ...
        tbody.innerHTML = `
            <tr class="hover:bg-gray-200 rounded-lg transition-colors">
                ...
                <td class="p-3 text-xl text-center">${producto.talla}</td>
                <td class="p-3 text-xl text-center">${producto.precio}</td>
                <td class="p-3 text-xl text-center">
                    <div class="grid grid-cols-2 gap-2 w-full">
                        <!-- tus botones aquí -->
                        <a href="#" onclick="window.location.href='actualizarProductoRopa/${producto.id}'" class="btn">
                        ...
                        <a href="#" onclick="eliminarProductoRopa(${producto.id})" class="btn">
...
function filtrarProductos() {
    ...
    fetch(`/filtrar-productos-ropa/?nombre=${nombre}&categoria=${categoria}&talla=${talla}`)
        .then(response => response.json())
        .then(data => {
            const tbody = document.querySelector('#datos tbody');
            tbody.innerHTML = '';
                ...
                data.forEach(producto => {
                    tbody.innerHTML += `
                        <tr class="hover:bg-gray-200 rounded-lg transition-colors">
                            ...
                            <td class="p-3 text-xl text-center">
                                <div class="grid grid-cols-2 gap-2 w-full">
                                    <a href="actualizarProductoRopa/${producto.id}" class="btn">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor"
                                            class="size-11 rounded-lg text-amber-300 hover:bg-gray-300 p-2 w-full">
                                    ...
                                    <a href="#" onclick="eliminarProductoRopa(${producto.id})" class="btn">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1.5" stroke="currentColor"
                                            class="size-11 rounded-lg text-red-500 hover:bg-gray-300 p-2 w-full">
                                            <path stroke-linecap="round" stroke-linejoin="round"
